...
export const DatabaseInspector: React.FC<DatabaseInspectorProps> = ({ onBackToMain }) => {
  ...
  // Set up sync service listeners for real-time updates
  useEffect(() => {
    ...
    const setupSync = async () => {
      try {
        ...
        const unsubscribe = syncService.onDatabaseChanged(() => {
          if (isSubscribed) {
            console.log('Database change detected, refreshing inventory...');
  ...
  // Handle cell editing
  const handleEditCell = async (rowIndex: number, columnKey: keyof InventoryItem, newValue: string) => {
    ...
    try {
      ...
      await databaseService.updateInventoryItemField(item.item_id!, columnKey as keyof InventoryItem, processedValue);
      ...
      await syncService.notifyInventoryUpdate(
  ...
  const handleDeleteRow = async (item: InventoryItem) => {
    if (window.confirm(`Are you sure you want to delete ${item.part_number}?`)) {
      try {
        ...
        await syncService.notifyInventoryUpdate(item, 'deleted');
      } catch (err) {
                  ...
                  >
                    {editingCell?.row === rowIndex && editingCell?.col === colIndex ? (
                      <input
                        ...
                        onBlur={(e) => handleEditCell(rowIndex, column.key, e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            handleEditCell(rowIndex, column.key, e.currentTarget.value);
...
export const InspectionPage = DatabaseInspector;
